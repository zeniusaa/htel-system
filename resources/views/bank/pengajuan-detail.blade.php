    ...
    <div class="py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
            ...
            <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {{-- FILE KTP --}}
                <div
                    class="bg-gray-50 rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center min-h-[400px] overflow-hidden">
                    @if (isset($dokumen['KTP']))
                        <iframe id="viewer-debitur" src="{{ Storage::url($dokumen['KTP']->file_path) }}"
        ...
        'Alamat' => $pengajuan->alamat,
                    ...
                    <div class="pt-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @if (isset($dokumen['KTP']))
                                <button
                                    onclick="gantiFile('viewer-debitur', '{{ Storage::url($dokumen['KTP']->file_path) }}', this)"
                            ...
                            @if (isset($dokumen['AKAD']))
                                <button
                                    onclick="gantiFile('viewer-debitur', '{{ Storage::url($dokumen['AKAD']->file_path) }}', this)"
                                    class="file-btn">
                                    AKAD
                                </button>
                            @endif
            ...
            <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div
                    ...
                    @if (isset($dokumen['SERTIFIKAT']))
                        <iframe src="{{ Storage::url($dokumen['SERTIFIKAT']->file_path) }}"
                            class="w-full h-96 rounded-lg">
                        </iframe>
                    @else
                        <div class="text-center py-8">
                            <span class="mt-2 block text-sm font-medium text-gray-400">File Sertifikat tidak
                                tersedia</span>
                        </div>
                    @endif
                </div>
            ...
            <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div
                    ...
                    @if (isset($dokumen['APHT']))
                        <iframe id="viewer-apht" src="{{ Storage::url($dokumen['APHT']->file_path) }}"
                            class="w-full h-96 rounded-lg">
                        </iframe>
                    @else
                        <div class="text-center py-8">
        ...
        'Kantor Cabang' => $pengajuan->kantor_pertanahan,
                    ...
                    {{-- Nominal --}}
                    <div
                        class="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg px-4 py-3 border-l-4 border-orange-400">
                    ...
                    <div class="pt-2">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @if (isset($dokumen['APHT']))
                                <button
                                    onclick="gantiFile('viewer-apht', '{{ Storage::url($dokumen['APHT']->file_path) }}', this)"
                                    class="file-btn active">
                                    APHT
                                </button>
                            @endif
                            ...
                            @if (isset($dokumen['SPA']))
                                <button
                                    onclick="gantiFile('viewer-apht', '{{ Storage::url($dokumen['SPA']->file_path) }}', this)"
                                    class="file-btn">
                                    SPA
                                </button>
                            @endif
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
            ...
            <div class="p-6">
                @if ($lamp13)
                    <iframe src="{{ Storage::url($lamp13->file_path) }}"
                    ...
                    @if ($pengajuan->status === 'DIAJUKAN')
                        <div class="mt-4">
                            <form method="POST" action="{{ route('bank.hapusLamp13', $pengajuan->id) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('Yakin ingin menghapus Lampiran 13?')"
                    ...
                    @else
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                @if ($sps)
                                    <iframe src="{{ Storage::url($sps->file_path) }}"
                                        class="w-full h-96 border border-gray-200 rounded-lg">
                                    </iframe>
                                @else
                            ...
                            <div>
                                ...
                                @if (in_array($pengajuan->status, ['DIBAYAR', 'TERBIT_SHT', 'SELESAI']) && $buktiBayar)
                                    <iframe src="{{ Storage::url($buktiBayar->file_path) }}"
                                        class="w-full h-96 border border-gray-200 rounded-lg">
                                    </iframe>
        ...
        @if ($pengajuan->status === 'DIBAYAR')
            <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
                ...
                <div class="p-6">
                    <form action="{{ route('bank.pengajuan.uploadSht', $pengajuan->id) }}" method="POST"
                        enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        ...
                        <div>
                            ...
                            <input type="file" name="sht" id="file_sht" accept=".pdf,.jpg,.jpeg,.png"
                                required
                                class="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:ring-2 focus:ring-[#00A39D] focus:border-transparent">
                        </div>
                        ...
                        <div id="previewContainer" class="hidden">
                            <iframe id="previewFrame"
                                class="hidden w-full h-[500px] border border-gray-200 rounded-lg"></iframe>
        ...
        @if (in_array($pengajuan->status, ['DIBAYAR', 'TERBIT_SHT', 'SELESAI']) && $sht)
            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
                ...
                <div class="p-6 space-y-4">
                    ...
                    @if ($extension === 'pdf')
                        <iframe src="{{ Storage::url($sht->file_path) }}"
                        ...
                        @endif
                    @else
                        <img src="{{ Storage::url($sht->file_path) }}" class="w-full rounded-lg shadow-sm">
                    @endif
                </div>
            </div>
