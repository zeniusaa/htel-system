...
class DashboardController extends Controller
{
    ...
    */
    ...
    {
        $query = Pengajuan::where('user_id', $user->id)->where('status', '!=', 'SELESAI');
    ...
    {
        $counts = Pengajuan::where('user_id', $userId)->selectRaw('status, COUNT(*) as total')->groupBy('status')->pluck('total', 'status');

        $statusList = ['UPLOAD', 'DIAJUKAN', 'DIPROSES', 'DITANGGUHKAN', 'PERINTAH_SETOR', 'DIBAYAR', 'TERBIT_SHT'];

        return collect($statusList)->mapWithKeys(fn($s) => [$s => $counts->get($s, 0)])->all();
    }
    ...
    */
    ...
    {
        $query = Pengajuan::with('ppat')->whereNotIn('status', ['UPLOAD', 'SELESAI']);

        if ($request->filled('search')) {
            $search = $request->search;

            $query->where(function ($q) use ($search) {
                $q->where('nama_debitur', 'like', "%$search%")->orWhereHas('ppat', function ($ppat) use ($search) {
    ...
    private function generateStatistikBank()
    {
        ...
        $statusList = ['DIAJUKAN', 'DIPROSES', 'DITANGGUHKAN', 'PERINTAH_SETOR', 'DIBAYAR', 'TERBIT_SHT'];
